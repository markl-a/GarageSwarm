...
class Worker(Base):
    ...
    machine_id = Column(
        ...
        unique=True,
        nullable=False,
        comment="Physical machine identifier",
    )

    machine_name = Column(
        String(100),
        nullable=False,
        comment="Human-readable machine name",
    )
    ...
    status = Column(
        ...
        default="offline",
        index=True,
        comment="Worker status: online | offline | busy | idle",
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('online', 'offline', 'busy', 'idle')",
            name="chk_worker_status",
        ),
    )

    # Relationships
    tasks = relationship("Task", back_populates="worker")
    user_associations = relationship("UserWorker", back_populates="worker", cascade="all, delete-orphan")
