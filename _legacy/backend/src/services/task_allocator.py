...
class TaskAllocator:
    ...
    async def _assign_subtask_to_worker(
        ...
        # Push task to worker via WebSocket (Redis pub/sub)
        await self._push_task_to_worker(subtask, worker)

    async def _push_task_to_worker(
        self,
        subtask: Subtask,
        worker: Worker
    ) -> bool:
        """
        Push a task assignment to a worker via Redis pub/sub.

        Args:
            subtask: Subtask to push
            worker: Worker to push to
        ...
        channel = f"worker:{worker.worker_id}:tasks"

        try:
            await self.redis.redis.publish(channel, json.dumps(message))
            logger.info(
                "Task pushed to worker",
                worker_id=str(worker.worker_id),
                subtask_id=str(subtask.subtask_id)
            )
            return True
